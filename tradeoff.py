...
def calc_class_weights(labels_path):
    ...
    weight_tensor = torch.tensor([weight for _, weight in class_weights]).float().to(device)
...
def run_train(val_path_fold, test_path_fold, features_path_fold, device, results_dir, model_dir, actions_dict,
    ...
    folder_class_labels_flag = "Weighted" if label_class_weights else "Regular"
    final_LSTM_flag = "LSTM" if final_lstm else "No-lstm"
    sample_size_flag = f"{sample_size}frames"
    ################## Flags to print ######################
    exts = [final_LSTM_flag, folder_class_labels_flag, sample_size_flag]
    ########################################################
    ...
    trainer = Trainer(num_layers_PG, num_layers_R, num_R, num_f_maps, features_dim, num_classes,
                      fold_num, fold_num, weighted=weighted_flag, kl=kl_flag, lstm=lstm_flag,
                      class_weights=class_weights if label_class_weights else None, final_lstm=final_lstm,
...
def main():
    ...
    parser.add_argument('--features_dim', default='1280', type=int) # default is 1280
    ...
    folds_to_run = 1 # 4 folds in total, change to 5 for range(5)

    fold_files = [(f"/datashare/APAS/folds/valid {i}.txt",
                   f"/datashare/APAS/folds/test {i}.txt",
                   f"/datashare/APAS/features/fold{i}/") for i in range(folds_to_run)]
    ...
    for val_path_fold, test_path_fold, features_path_fold in fold_files:
        for sample_size in [16]:
            run_train(val_path_fold=val_path_fold, test_path_fold=test_path_fold,
                      ...
                      lstm_flag=False,
                      weighted_flag=False,
                      final_lstm=True, sample_size=sample_size)
