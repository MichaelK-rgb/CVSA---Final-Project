...
class MS_TCN2_LSTM(nn.Module):
    def __init__(self, num_layers_PG, num_layers_R, num_R, num_f_maps, dim, num_classes, sample_size=5):
        ...
        #   Added by us
        #self.PG_kin = Prediction_Generation(num_layers_PG, num_f_maps, 36, num_classes)
        self.lstm = nn.LSTM(input_size=6, batch_first=True, hidden_size=128, num_layers=1, dropout=0.1, bidirectional=True)
        self.hidden_to_label = nn.Linear(in_features=256, out_features=128)
    ...
    def forward(self, x):
        ...
        #outputs = outputs * (1 - 0.2) + outputs_kin * 0.2
                
        self.sample_size=5
        out = F.interpolate(F.softmax(out, dim=1), int(out.shape[-1] / self.sample_size))
        out = self.lstm(out.transpose(2, 1))[0].squeeze(0)
        out = self.hidden_to_label(out)
        out = self.relu(out)
        out = self.fc(out).unsqueeze(0).transpose(1, 2)
        outputs = torch.cat((outputs, F.interpolate(out, outputs.shape[-1]).unsqueeze(0)), dim=0)
        return outputs


class MS_TCN2(nn.Module):
    def __init__(self, num_layers_PG, num_layers_R, num_R, num_f_maps, dim, num_classes, weighted=False, lstm=False):
        ...
        self.lstm = nn.LSTM(input_size=6, batch_first=True, hidden_size=64, num_layers=3, dropout=0.2, bidirectional=True)
    ...
    def forward(self, x):
        ...
        outputs = out.unsqueeze(0)
        for i, R in enumerate(self.Rs):
            if self.lstm_flag and i == 1 and not self.weighted:
                out = self.lstm(F.softmax(out, dim=1).transpose(2, 1))[0].squeeze(0)
...
class Trainer:
    def __init__(self, num_layers_PG, num_layers_R, num_R, num_f_maps, dim, num_classes, dataset, split,
                 weighted=False, kl=False, class_weights=None, lstm=False, final_lstm=False, sample_size=5):
        if final_lstm:
            self.model = MS_TCN2_LSTM(num_layers_PG, num_layers_R, num_R, num_f_maps, dim, num_classes,
        ...
        self.class_weights_str = "Weighted" if self.class_weights else "NoClassWeighting"
        self.lstm = lstm
        self.lstm_str = "MiddleLSTM" if self.lstm else "MiddleNoLSTM"
        self.final_lstm = final_lstm
        self.final_lstm_str = "LSTM" if self.final_lstm else "No-lstm"
        self.sample_size = sample_size
        self.sample_size_str = f"{sample_size}frames"
        ext = [self.class_weights_str, self.final_lstm_str, self.sample_size_str]
    ...
    def train(self, save_dir, batch_gen_train, batch_gen_val, num_epochs, batch_size, learning_rate, device, clogger):
        ...
        for epoch in tqdm(range(num_epochs)):
            ...
            while batch_gen_train.has_next():
                ...
                x_softmax = torch.softmax(predictions, dim=-1)
                # sum along the last dimension to obtain a tensor of shape [5,1,6]
                x_sum = torch.sum(x_softmax, dim=-1)
                loss = 0
                ...
                #print(torch.max(predictions[-1].data, 1))
                _, predicted = torch.max(predictions[-1].data, 1)
                #_, b = torch.max(predictions[1].data, 1)
                #print(predicted)
                #_, a= torch.max(predictions[-1].data, 2)
                #print(predictions[-1])
                
                """
                for pred_tensor in predictions[-1]:
                    for frame in range(predictions.shape[3]):
                        
                        #print(pred_tensor[0][frame][0])
                        #print('second')
                        #print(pred_tensor[1][frame][0])

                        frame_preds = [pred_tensor[0][frame].item(), pred_tensor[1][frame].item(), pred_tensor[2][frame].item(),\
                         pred_tensor[3][frame].item(), pred_tensor[4][frame].item(), pred_tensor[5][frame].item()]
                         
                        # softmax on predictions to get confidence level for each frame's class prediction
                        output = np.exp(frame_preds)/sum(np.exp(frame_preds))
                        #print(output.max())
                        
                        conf_level_lst.append(output.max())
                
                
                print(conf_level_lst[-10:])
                # print(len(conf_level_lst))
    ...
    def predict(self, model_dir, results_dir, features_path, vid_list_file, epoch, actions_dict, device, sample_rate):
        ...
        with torch.no_grad():
            ...
            for vid in list_of_vids:
                ...
                """
                f_name2 = vid.split('/')[-1].split('.')[0]
                f_ptr2 = open(results_dir + "/" + f_name + 'conf', "w")
                f_ptr2.write("### Frame level confidence: ###\n")
                f_ptr2.write(' '.join(conf_level_lst))
                
                f_ptr2.close()
                
                
                """
                open_pkl = open(results_dir + "/" + f_name + 'conf' + ".pickle", "wb")
                pickle.dump(conf_level_lst, open_pkl)
                open_pkl.close()
