...
def plot_segments(pred, gt, index):
    ...
    colors = ['red', 'orange', 'green', 'blue', 'pink', 'black']
    ...
    plt.vlines(x=index-110, ymin=1.2, ymax=1.5,
               colors='darkblue')
    plt.ylim(-2, 3)
    plt.text(0, 0.5, "Ground True", fontsize=9,verticalalignment='center', transform= fig.transFigure)
    plt.text(0, 0.3, "Prediction", fontsize=9, transform= fig.transFigure)
    plt.axis("off")
    fig.canvas.draw()
...
def main():
    ...
    video_names = [f'P022_tissue2']
    #frames = [f'folder{i}' for i in [1,2,3]]
    ...
    for ind, vid in enumerate(video_names):
        image_folder = f'/datashare/APAS/frames/{vid}_side'
        #image_folder = f'{frames[ind]}'
        label_path = f"/home/student/MS-TCN++/results/exp35/fold0/Final-GRU_ClassWeighted_Sample size 5/{vid}"

        #gt_path = f"/datashare/APAS/transcriptions_gestures/{vid}.txt"
        gt_path = f"transcriptions_gestures/{vid}.txt"
        ...
        for index, (image, pred, gt) in tqdm(enumerate(zip(images, predictions, gts))):
            ...
            draw_text(image_with_segmentation, "Prediction: " + gestures[pred], pos=(0, 30 + 480), text_color=(255, 0, 0))
